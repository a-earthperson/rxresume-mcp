...
def _apply_project_defaults(payload: Dict[str, Any]) -> Dict[str, Any]:
...
def _reshape_project_items(payload: Any) -> Any:
    if isinstance(payload, list):
        return [_reshape_project_item(item) for item in payload]
    return payload


def _build_project_update_ops(item: ProjectItemInput) -> List[Dict[str, Any]]:
    payload = item.model_dump(exclude_none=True)
    ...
    for field, (target, transform) in field_values.items():
        ...
        ops.append(
            patch_ops.op_replace(
                patch_ops.path_section_item_field("projects", item_id, target),
    ...
    if summary_provided or highlights_provided:
        ...
        ops.append(
            patch_ops.op_replace(
                patch_ops.path_section_item_field("projects", item_id, "description"),
                description,
            )
        )
    ...
    if "url" in payload:
        ...
        base = patch_ops.path_section_item_field("projects", item_id, "website")
        for key, value in website_payload.items():
            ops.append(patch_ops.op_replace(f"{base}/{key}", value))

    if payload:
        raise ValueError(f"Unexpected fields in project update: {sorted(payload)}")
    if not ops:
...
def register_project_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool(
        name="resume.section.project.list",
        description="List project items for a resume.",
    )
    async def list_project(
        ...
        async def _operation(client: RxResumeClient) -> Any:
            ...
            items = extract_section_items(resume, "projects", label="Projects")
            return _reshape_project_items(items)

        return await execute_rxresume_operation(
            operation_name=f"list project: {resume_id}",
    ...
    async def add_project(
        ctx: Context,
        resume_id: str = Field(description="Resume ID"),
        items: Optional[ProjectItemsInput] = Field(
            default=None,
            description="Project item or list of items to add.",
        ),
    ) -> Dict[str, Any]:
        async def _operation(client: RxResumeClient) -> Any:
            if items is None:
            ...
            ops: List[Dict[str, Any]] = []
            for item in coerce_model_items(items, ProjectItemInput):
                payload = _prepare_project_item(item, created_ids)
                ops.append(
                    patch_ops.op_add(
                        patch_ops.path_section_items_append("projects"), payload
                    )
                )
            result = await apply_section_item_patch(
                client, resume_id, "projects", ops, label="Projects"
            )
            return _reshape_project_items(result)

        return await execute_rxresume_operation(
            operation_name=f"add project: {resume_id}",
            operation_func=_operation,
            ctx=ctx,
        )
    ...
    @mcp.tool(
        ...
        description="Remove one or more project items by id.",
    )
    async def remove_project(
        ctx: Context,
        resume_id: str = Field(description="Resume ID"),
        item_ids: Optional[ProjectItemIdsInput] = Field(
        ...
        async def _operation(client: RxResumeClient) -> Any:
            ...
            for item_id in coerce_item_ids(item_ids):
                ops.append(
                    patch_ops.op_remove(
                        patch_ops.path_section_item("projects", item_id)
                    )
                )
            result = await apply_section_item_patch(
                client, resume_id, "projects", ops, label="Projects"
        ...
        return await execute_rxresume_operation(
            operation_name=f"remove project: {resume_id}",
            operation_func=_operation,
            ctx=ctx,
        )
    ...
    @mcp.tool(
        ...
        description=(
            "Update one or more project items by id. "
    ...
    async def update_project(
        ...
        items: Optional[ProjectItemsInput] = Field(
            default=None,
            description="Project item or list of items to update.",
        ),
    ) -> Dict[str, Any]:
        async def _operation(client: RxResumeClient) -> Any:
            if items is None:
            ...
            ops: List[Dict[str, Any]] = []
            for item in coerce_model_items(items, ProjectItemInput):
                ops.extend(_build_project_update_ops(item))
            result = await apply_section_item_patch(
                client, resume_id, "projects", ops, label="Projects"
            )
            return _reshape_project_items(result)
        ...
        return await execute_rxresume_operation(
            operation_name=f"update project: {resume_id}",
            operation_func=_operation,
            ctx=ctx,
        )
