...
Across full CRUD coverage of the tool surface (resume documents, basics, all section item types, and exports), the dominant usability blocker is **schema non-round‑trippability**: many endpoints **accept one set of field names** (e.g., `website`, `headline`, `company`, `title`, `date`, `language`, `fluency`, `phone`) but **return a different set** (e.g., `url`, `label`, `name`, `period`, `proficiency`, `contact`). This breaks the standard agent loop of *read → modify JSON → write*, and it is particularly harmful for small/self-hosted models that rely heavily on copying prior tool outputs.
...
- returned `url`, but input expects `website`
- returned `label`, but input expects `headline`
- returned `name`, but input expects `company`/`title`/`language`/`organization`
- returned `period`, but input expects `date`
- returned `proficiency`, but input expects `fluency`
- returned `contact`, but input expects `phone`
...
|---:|---|---|---|---|
| 1 | 1) Called `profile.item.create` with input `website`.<br>2) Response returned `url` field, not `website`.<br>3) Same pattern observed across basics/language/award/etc with different renames.<br>4) Attempting read→edit→write using returned JSON would fail or require manual mapping. | **Non-round‑trippable schemas**: request field names differ from response field names (e.g., `website→url`, `headline→label`, `company/title/language/organization→name`, `date→period`, `phone→contact`). | **High**: small models heavily rely on copying tool outputs; mapping tables increase hallucination/error risk and force extra tool calls. | Make all endpoints **return the same schema they accept** (canonical). If internal schema differs, translate responses too (not just requests). Consider returning both `url` and `website` during transition. |
| 2 | 1) Tried `education.item.create` with `grade:null` (schema says nullable).<br>2) Backend returned `expected string, received null`.<br>3) Similar failures reproduced for project `period`, award `date/description`, certification `date/description`, reference `position/phone/description`, volunteer `location/period`, etc.<br>4) Empty strings succeed. | **Declared optional/nullable fields are effectively required to be strings** on create; backend rejects `null` for many “optional” strings. | **High**: small models will follow the manifest and send nulls → repeated 400 loops. | Fix schemas to match reality: mark required fields required; remove `| null` where backend rejects it. Alternatively, wrapper should **omit absent fields** (don’t serialize them as null) or auto-coerce `null→""` where appropriate. |
...
**Reasoning steps**
1) The tool already seems to translate *inputs* (`website→url`, `headline→label`, etc.).
2) It does **not** translate *outputs*, forcing agents to learn internal names.
3) Round-trip fails and is the biggest recurring usability issue.

**Proposal**
- Define a canonical external schema (pick one and commit):
  - Either JSON Resume–like (`url`, `label`, etc.), **or**
  - The current MCP manifest schema (`website`, `headline`, `company`, `title`, `date`, etc.).
...
#### B3) Schema mismatch example (profile)
**Create input uses `website`**
```json
{"items":{"id":null,"network":"X","username":"eva","website":"https://x.com/eva"}}
```
**Response returns `url`**
```json
{"id":"...","network":"X","username":"eva","url":"https://x.com/eva"}
```
...
**Input**
```json
{"items":{"id":"does-not-exist","company":"Nope"}}
```
**Output**
```json
{
  "status":"error",
  "error":"HTTP 404: ... 'PATCH_TARGET_NOT_FOUND' ... 'path': '/data/sections/experience/items/id/does-not-exist/company'"
}
```

---

### C) Observed input→output field mappings (partial)

- Basics: `headline → label`, `website → url`
- Profile: `website → url`
- Experience: `company → name`, `website → url`
- Language: `language → name`, `fluency → proficiency`
- Award/Certification: `title → name`, `date → period`, `description → summary`, `website → url`
- Reference: `phone → contact`, `website → url`, `description → summary`

This mapping is not exposed as an explicit contract, forcing agents to infer it.